import 'package:flutter/cupertino.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

class FontStyles {
  static const fontFamily = 'Montserrat';
  static const hpFontFamily = 'Biergarten';
  static final font31 = 31.0.sp;
  static const font25 = 25.0;
  static const font19 = 19.0;
  static const font18 = 18.0;
  static const font17 = 17.0;
  static const font14 = 14.0;
  static const font13 = 13.0;
  static const font12 = 12.0;
  static const font11 = 11.0;
  static const font10 = 10.0;
  static const font9 = 9.0;

  static TextStyle hpFontBig() {
    return const TextStyle(
      fontSize: 50,
      fontWeight: FontWeight.w500,
      fontFamily: hpFontFamily,
    );
  }

  static TextStyle hpFontSmall() {
    return const TextStyle(
      fontSize: 35,
      fontWeight: FontWeight.w500,
      fontFamily: hpFontFamily,
    );
  }

  static TextStyle montserratExtraBold11() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font31.sp);
  }

  static TextStyle montserratExtraBold12() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font12.sp);
  }

  static TextStyle montserratExtraBold13() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font13.sp);
  }

  static TextStyle montserratExtraBold14() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font14.sp);
  }

  static TextStyle montserratExtraBold17() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font17.sp);
  }

  static TextStyle montserratExtraBold18() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font18.sp);
  }

  static TextStyle montserratExtraBold19() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font19.sp);
  }

  static TextStyle montserratExtraBold25() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font25.sp);
  }

  static TextStyle montserratExtraBold31() {
    return TextStyle(
        fontWeight: FontWeight.w800,
        fontFamily: fontFamily,
        fontSize: font31.sp);
  }

  static TextStyle montserratRegular14() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font14.sp,
    );
  }

  static TextStyle montserratRegular11() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font11.sp,
    );
  }

  static TextStyle montserratRegular10() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font10.sp,
    );
  }

  static TextStyle montserratRegular9() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font9.sp,
    );
  }

  static TextStyle montserratRegular12() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font12.sp,
    );
  }

  static TextStyle montserratRegular13() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font13.sp,
    );
  }

  static TextStyle montserratRegular17() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font17.sp,
    );
  }

  static TextStyle montserratRegular18() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font18.sp,
    );
  }

  static TextStyle montserratRegular19() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font19.sp,
    );
  }

  static TextStyle montserratRegular25() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font25.sp,
    );
  }

  static TextStyle montserratRegular31() {
    return TextStyle(
      fontWeight: FontWeight.w400,
      fontFamily: fontFamily,
      fontSize: font31.sp,
    );
  }

  static TextStyle montserratBold17() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font17.sp);
  }

  static TextStyle montserratBold31() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font31.sp);
  }

  static TextStyle montserratBold25() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font25.sp);
  }

  static TextStyle montserratBold19() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font19.sp);
  }

  static TextStyle montserratBold18() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font18.sp);
  }

  static TextStyle montserratBold14() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font14.sp);
  }

  static TextStyle montserratBold13() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font13.sp);
  }

  static TextStyle montserratBold12() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font12.sp);
  }

  static TextStyle montserratBold11() {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontFamily: fontFamily,
        fontSize: font11.sp);
  }

  static TextStyle montserratSemiBold9() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font9.sp);
  }

  static TextStyle montserratSemiBold10() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font10.sp);
  }

  static TextStyle montserratSemiBold11() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font11.sp);
  }

  static TextStyle montserratSemiBold12() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font12.sp);
  }

  static TextStyle montserratSemiBold13() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font13.sp);
  }

  static TextStyle montserratSemiBold14() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font14.sp);
  }

  static TextStyle montserratSemiBold17() {
    return TextStyle(
      fontWeight: FontWeight.w600,
      fontFamily: fontFamily,
      fontSize: font17.sp,
    );
  }

  static TextStyle montserratSemiBold18() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font18.sp);
  }

  static TextStyle montserratSemiBold19() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font19.sp);
  }

  static TextStyle montserratSemiBold25() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font25.sp);
  }

  static TextStyle montserratSemiBold31() {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontFamily: fontFamily,
        fontSize: font31.sp);
  }
  // static TextStyle montserratExtraBold() {
  //   return const TextStyle(fontWeight: FontWeight.w800, fontFamily: fontFamily);
  // }
}
